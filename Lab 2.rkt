...
(define (last-name name) (car (cdr name)))
...
(define (IDnumber student) (car (car student)))

;; Returns '((number street apt) (city state zip))
(define (address student) (car (cdr (cdr (cdr (cdr student))))))

;; Returns the 'GPA field
(define (GPA student) (cdr(cdr(car (cdr (cdr (cdr student)))))))

;; Returns the 'state field
(define (state student) (car(cdr(car(cdr(car(cdr(cdr(cdr(cdr student))))))))))
...
(define (pig-latin word) (append (append (cdr word) (list(car word))) ay))
...
(define (yoda 3-words) (append (cdr (cdr 3-words)) (append(list(car 3-words)) (list(car(cdr 3-words))))))
