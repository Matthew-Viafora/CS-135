...
(define (mod-exp b e m) (cond ((equal? e 0) 1)
                              ((even? e) (modulo (expt (mod-exp b (/ e 2) m) 2) m))
                              ((odd? e) (modulo (* (mod-exp b (- e 1) m) b) m))))
...
(define (encrypt plaintext key) (cond ((equal? plaintext "") '())
                                      (else (cons (mod-exp (char->integer (car (string->list plaintext))) (car(cdr key)) (car key)) (encrypt (list->string (cdr (string->list plaintext))) key)))))
...
(define (decrypt ciphertext key) (cond ((equal? ciphertext '()) "")
                                       (else (string-append (list->string (list (integer->char (mod-exp (car ciphertext) (car (cdr key)) (car key))))) (decrypt (cdr ciphertext) key)))))
